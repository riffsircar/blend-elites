...
mutation_prob = 0.3
elems = True # False for tile metrics
runs = 1
generations = 100000
tag = 'elem'
BC1, BC2 = 'symmetry', 'similarity'
FC = 0
CONV = 1
TYPE = FC  # 0 - fc, 1 - conv

folders = {'smb':smb_folder,'ki':ki_folder,'mm':mm_folder,'ng':ng_folder,'cv':cv_folder,'met':met_folder,'blend':None,'blend_aff_skm':None}
out_folders = {'smb':'out_smb','ki':'out_ki','mm':'out_mm','ng':'out_ng','cv':'out_cv','blend':'out_blend','met':'out_met','blend_aff_skm':'out_blend_aff_skm'}
label_sizes = {'smb':5,'ki':4,'mm':5,'ng':5,'cv':7,'blend':10,'met':5,'blend_aff_skm':10}
...
all_images = {'smb':smb_ki_images, 'ki':smb_ki_images, 'mm':mm_images,'blend':smb_ki_mm_images,'ng':ng_images,'cv':cv_images,'met':mm_images,'blend_aff_skm':aff_images}
...
#bc_metrics['difficulty'] = max(diff)
...
for i in range(runs):
    ...
    for i, z in enumerate(init_pop):
        ...
        if archive[(c1,c2)] is None:
            ...
            if not GAME.startswith('blend'):
                cur_z, cur_score = archive[(c1,c2)]
                if this_score > cur_score:
                    archive[(c1,c2)] = z, this_score
            else:
                cur_z, score, label = archive[(c1,c2)]
                if this_score > score:
                    archive[(c1,c2)] = z, this_score, this_label
                if this_score == 1.0:
                    new_label = int(label,2) | int(this_label,2)
                    new_label = '{:b}'.format(new_label)
                    #new_label = f"{new_label:b}".zfill(3)
                    new_label = new_label.zfill(3)
                    archive[(c1,c2)] = z, this_score, new_label
    ...
    for g in range(generations):
        ...
        if archive[(cell_1,cell_2)] is None:
            if not GAME.startswith('blend'):
                archive[(cell_1,cell_2)] = z_child, this_score
            else:
                archive[(cell_1,cell_2)] = z_child, this_score, this_label
        else:
            if not GAME.startswith('blend'):
                cur_z, cur_score = archive[(cell_1,cell_2)]
                if this_score > cur_score:
                    archive[(cell_1,cell_2)] = z_child, this_score
            else:
                cur_z, cur_score, cur_label = archive[(cell_1,cell_2)]
                if this_score > cur_score:
                    archive[(cell_1,cell_2)] = z_child, this_score, this_label
                if this_score == 1.0:
                    ...
                    #new_label = f"{new_label:b}".zfill(3)
                    new_label = new_label.zfill(3)
                    archive[(cell_1,cell_2)] = z_child, this_score, new_label
...
if not elems:
    ...
    fig = plt.figure(figsize=(bc2_range/2,bc1_range/6))
